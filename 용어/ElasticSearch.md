...
### 2. 노드
   ...
   - 클러스터인 경우 마스터 전용 노드와 데이터 전용 노드를 분리하는 것이 좋다.
...
### 3. 샤딩 (Sharding)
   ...
   - shard
     - ES를 비롯한 많은 수의 분산 데이터베이스에서 사용하고 있으며, 데이터를 저장할 때 나누어진  조각에 대한 단위이다. 
- 이때 각각의 shard는 데이터에 대한 복사본이 아니라 데이터 그 자체이다. 
...
[홈페이지](<https://www.elastic.co/kr/products/elasticsearch>)에 있는 zip 파일이나 install 명령어로 쉽게 설치 할 수 있다.
